{
    ...
    let CATE_NO_box = new Array();

    delete_button.addEventListener("click", (e) => {
        
        for(let i = 0 ; i < delete_check.length ; i++){
            if( delete_check[i].checked ){
                CATE_NO_box.push(delete_check[i].name);
            }
        }

        console.log(CATE_NO_box);
        ...
        fetch(`https://click.ecc.ac.jp/ecc/sys2_iesk2bc_a/test_html/WebAPI/Admin_CategoryAPI.php`, {
            method: "POST",
            body: new URLSearchParams({
                Cate_No : JSON.stringify(CATE_NO_box)
            })
        })
        .then((response)=>{
    ...
    Insert_button.addEventListener("click", (e) => {
        window.location.href = "Admin_NewInsert.php?TABLE=CATEGORY";
    });
